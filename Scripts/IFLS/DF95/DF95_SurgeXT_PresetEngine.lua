-- @description Surge XT Preset Engine (Role + Flavor Based)
-- @author You
-- @version 1.0
-- @about
--   Rolle/Flavor-basierte Preset Engine für Surge XT:
--     • erkennt Surge XT über FX-Namen
--     • setzt Filter-Cutoff, Resonance, Env, Unison/Detune, Drive, Macros
--     • pro Synth-Rolle (bass/lead/pad/keys/pluck/fx/drone)
--     • Flavor: "clean", "color", "aggressive", "weird"
...
function M.IsSurgeFXName(fxname)
  if not fxname then return false end
  local l = fxname:lower()
  -- typische Varianten: "Surge XT", "Surge Synth Team - Surge XT", etc.
  return l:find("surge xt", 1, true) ~= nil or l:find("surge", 1, true) ~= nil
end

local function set_filter1(tr, fx_idx, cutoff, res)
  -- Surge XT Filter 1 Cutoff / Resonance (Tooltips in der GUI zeigen diese Namen)
  set_param_like(tr, fx_idx, { "filter 1 cutoff", "cutoff", "f1 cutoff" }, cutoff)
  set_param_like(tr, fx_idx, { "filter 1 resonance", "resonance", "f1 res" }, res)
end

local function set_amp_env_adsr(tr, fx_idx, a, d, s, rls)
  -- Amplitude Envelope (Amp EG) / Env1/Env2, je nach Host-Param-Namen
  set_param_like(tr, fx_idx, { "amp env attack", "env 1 attack", "attack" }, a)
  set_param_like(tr, fx_idx, { "amp env decay",  "env 1 decay",  "decay" },  d)
  set_param_like(tr, fx_idx, { "amp env sustain","env 1 sustain","sustain"}, s)
  set_param_like(tr, fx_idx, { "amp env release","env 1 release","release"}, rls)
end

local function set_unison(tr, fx_idx, amount)
  -- Surge XT hat Unison- und Detune-Parameter pro Osc; wir gehen über Namen
  set_param_like(tr, fx_idx, { "unison", "detune", "spread" }, amount)
end

local function set_drive(tr, fx_idx, amount)
  -- Waveshaper / Drive im Filterblock oder global
  set_param_like(tr, fx_idx, { "drive", "distortion", "shaper", "saturation" }, amount)
end

local function set_osc_mix(tr, fx_idx, osc1, osc2)
  set_param_like(tr, fx_idx, { "osc 1 level", "osc1 level", "osc1 gain" }, osc1)
  set_param_like(tr, fx_idx, { "osc 2 level", "osc2 level", "osc2 gain" }, osc2)
end
...
local function apply_for_role_and_flavor(tr, fx_idx, role, flavor)
  ...
  if role == "bass" then
    if flavor == "clean" then
      -- Sub/Low Bass, wenig Obertöne
      set_osc_mix(tr, fx_idx, 1.0, 0.0)
      set_filter1(tr, fx_idx, 0.2, 0.2)
      set_amp_env_adsr(tr, fx_idx, 0.005, 0.15, 0.85, 0.1)
    ...
    elseif flavor == "weird" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.01, 0.25, 0.7, 0.2)
  ...
  elseif role == "lead" then
    if flavor == "clean" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.01, 0.18, 0.85, 0.2)
      set_unison(tr, fx_idx, 0.2)
      set_drive(tr, fx_idx, 0.1)
    elseif flavor == "color" then
      set_osc_mix(tr, fx_idx, 0.9, 0.5)
      set_filter1(tr, fx_idx, 0.65, 0.25)
      set_amp_env_adsr(tr, fx_idx, 0.005, 0.18, 0.8, 0.18)
    ...
    elseif flavor == "aggressive" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.003, 0.16, 0.75, 0.18)
      set_unison(tr, fx_idx, 0.4)
      set_drive(tr, fx_idx, 0.5)
    ...
    elseif flavor == "weird" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.02, 0.3, 0.7, 0.25)
  ...
  elseif role == "pad" then
    if flavor == "clean" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.12, 0.6, 0.9, 0.85)
      set_unison(tr, fx_idx, 0.4)
      set_drive(tr, fx_idx, 0.1)
    elseif flavor == "color" then
      set_osc_mix(tr, fx_idx, 0.85, 0.6)
      set_filter1(tr, fx_idx, 0.5, 0.35)
      set_amp_env_adsr(tr, fx_idx, 0.15, 0.7, 0.9, 0.9)
      set_unison(tr, fx_idx, 0.5)
    ...
    elseif flavor == "aggressive" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.18, 0.8, 0.9, 0.9)
      set_unison(tr, fx_idx, 0.6)
      set_drive(tr, fx_idx, 0.25)
    elseif flavor == "weird" then
      set_osc_mix(tr, fx_idx, 0.7, 0.8)
      set_filter1(tr, fx_idx, 0.6, 0.5)
      set_amp_env_adsr(tr, fx_idx, 0.2, 0.9, 0.9, 1.0)
  ...
  elseif role == "keys" then
    ...
    elseif flavor == "color" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.008, 0.22, 0.7, 0.2)
      set_unison(tr, fx_idx, 0.25)
      set_drive(tr, fx_idx, 0.2)
    elseif flavor == "aggressive" then
      set_osc_mix(tr, fx_idx, 0.9, 0.6)
      set_filter1(tr, fx_idx, 0.65, 0.35)
      set_amp_env_adsr(tr, fx_idx, 0.005, 0.2, 0.65, 0.18)
      set_unison(tr, fx_idx, 0.3)
      set_drive(tr, fx_idx, 0.35)
    elseif flavor == "weird" then
      set_osc_mix(tr, fx_idx, 0.8, 0.8)
      set_filter1(tr, fx_idx, 0.6, 0.5)
      set_amp_env_adsr(tr, fx_idx, 0.008, 0.25, 0.6, 0.2)
  ...
  elseif role == "pluck" then
    if flavor == "clean" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.002, 0.12, 0.35, 0.12)
      set_unison(tr, fx_idx, 0.25)
    elseif flavor == "color" then
      set_osc_mix(tr, fx_idx, 0.85, 0.4)
      set_filter1(tr, fx_idx, 0.65, 0.3)
      set_amp_env_adsr(tr, fx_idx, 0.002, 0.14, 0.4, 0.14)
      set_unison(tr, fx_idx, 0.3)
      set_drive(tr, fx_idx, 0.25)
    elseif flavor == "aggressive" then
      set_osc_mix(tr, fx_idx, 0.9, 0.5)
      set_filter1(tr, fx_idx, 0.7, 0.35)
      set_amp_env_adsr(tr, fx_idx, 0.001, 0.12, 0.3, 0.12)
      set_unison(tr, fx_idx, 0.35)
      set_drive(tr, fx_idx, 0.35)
    elseif flavor == "weird" then
      set_osc_mix(tr, fx_idx, 0.8, 0.7)
      set_filter1(tr, fx_idx, 0.7, 0.4)
      set_amp_env_adsr(tr, fx_idx, 0.002, 0.15, 0.4, 0.16)
      set_unison(tr, fx_idx, 0.4)
      set_drive(tr, fx_idx, 0.4)
  ...
  elseif role == "fx" then
    if flavor == "clean" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.02, 0.5, 0.7, 0.6)
      set_unison(tr, fx_idx, 0.5)
      set_drive(tr, fx_idx, 0.3)
    elseif flavor == "color" then
      set_osc_mix(tr, fx_idx, 0.8, 0.8)
      set_filter1(tr, fx_idx, 0.6, 0.5)
      set_amp_env_adsr(tr, fx_idx, 0.03, 0.6, 0.75, 0.65)
    ...
    elseif flavor == "aggressive" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.04, 0.7, 0.8, 0.7)
      set_unison(tr, fx_idx, 0.6)
      set_drive(tr, fx_idx, 0.5)
      set_macro(tr, fx_idx, 2, 0.8)
    elseif flavor == "weird" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.05, 0.8, 0.8, 0.8)
  ...
  elseif role == "drone" then
    if flavor == "clean" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.3, 1.0, 1.0, 1.0)
      set_unison(tr, fx_idx, 0.5)
      set_drive(tr, fx_idx, 0.15)
    elseif flavor == "color" then
      set_osc_mix(tr, fx_idx, 0.9, 0.6)
      set_filter1(tr, fx_idx, 0.4, 0.45)
      set_amp_env_adsr(tr, fx_idx, 0.35, 1.0, 1.0, 1.0)
      set_unison(tr, fx_idx, 0.6)
      set_drive(tr, fx_idx, 0.2)
      set_macro(tr, fx_idx, 1, 0.5)
    elseif flavor == "aggressive" then
      ...
      set_amp_env_adsr(tr, fx_idx, 0.4, 1.0, 1.0, 1.0)
      set_unison(tr, fx_idx, 0.7)
      set_drive(tr, fx_idx, 0.3)
    elseif flavor == "weird" then
      set_osc_mix(tr, fx_idx, 0.9, 0.9)
      set_filter1(tr, fx_idx, 0.5, 0.6)
      set_amp_env_adsr(tr, fx_idx, 0.45, 1.0, 1.0, 1.0)
