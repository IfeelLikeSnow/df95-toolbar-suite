...
-- @description Master Bus Selector (categories + auto-mapping to filenames)
-- @version 2.2
...
local function apply_chain(path, append)
  ...
  local ok, err = C.write_chunk_fxchain(master, txt, false)
  if not ok then r.ShowMessageBox("Fehler beim Laden:\n"..(err or "?"),"DF95 Master",0) end
end

local function 

-- Optional helper: apply chain + immediately run DF95 Safety & Loudness Menu on Master
local function DF95_Master_ApplyChain_WithSafety(path)
  apply_chain(path, false)
  local sep = package.config:sub(1,1)
  ...
  if f then f:close(); dofile(safety_path) end
end

build_and_show_menu()
  local cats = C.list_by_category("Master")
  local items = {"# DF95 Master Bus"}
  local index_to_path = {}
