...
-- @description Master Chain Browser
-- @version 1.3
-- @author DF95
-- @about
...
local ctx = r.ImGui_CreateContext("Master Chain Browser", 0)
...
local function apply_chain_to_selected(chain_name)
  ...
  if sel_count == 0 then
    r.ShowMessageBox("Keine selektierten Tracks. Bitte wähle einen oder mehrere Tracks aus.", "Master Chain Browser", 0)
    return
  end
  r.Undo_BeginBlock()
  ...
  r.Undo_EndBlock("Master Chain Browser: Apply " .. chain_name, -1)
end
...
local function loop()
  local visible, open = r.ImGui_Begin(ctx, "Master Chain Browser", true)
  if visible then
    if #cat_list == 0 then
