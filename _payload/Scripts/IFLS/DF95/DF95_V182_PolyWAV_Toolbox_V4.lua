...
-- @description DF95_V182 PolyWAV Toolbox V4 – ZoomF6 + FXMatrix + SampleDB
-- @author DF95
-- @version 1.0
-- @about
...
local function save_sampledb_multi_ucs(db, db_path)
  ...
  if not f then
    r.ShowMessageBox(
      ...
      "DF95 PolyWAV Toolbox V4 – SampleDB Bridge", 0)
    return false
  end
...
-- FX-Matrix nur auf selektierte Tracks anwenden (ohne Explode, funktioniert z.B. mit beliebigen Fieldrecordings)
local function fxmatrix_apply_to_selected_tracks()
  local cats = fxmatrix_scan_fxchains()
  local sel_cnt = r.CountSelectedTracks(0)
  if sel_cnt == 0 then
    r.ShowMessageBox("Keine Tracks selektiert.\nBitte Tracks auswählen, auf die die FX-Matrix angewendet werden soll.", "DF95 PolyWAV Toolbox V4 – FXMatrix", 0)
    return
  end
...
r.Undo_EndBlock("DF95 PolyWAV Toolbox V4 – Explode + ZoomF6 + SampleDB", -1)
end
...
local function ensure_imgui()
  ...
  if not r.ImGui_CreateContext then
    r.ShowMessageBox(
      ...
      "DF95 PolyWAV Toolbox V4", 0)
    return nil
  end
  ctx = r.ImGui_CreateContext("DF95 PolyWAV Toolbox V4")
  return ctx
end
...
local function loop()
  ...
  local visible, open = r.ImGui_Begin(ctx, "DF95 PolyWAV Toolbox V4 – ZoomF6 + FXMatrix", true)
  ...
  if visible then
    ...
    r.ImGui_Text(ctx, "→ nutzt REAPER-Action + DF95-Mapping/Bridge")
    ...
    if r.ImGui_Button(ctx, "FX-Matrix nur auf selektierte Tracks anwenden", 0, 0) then
