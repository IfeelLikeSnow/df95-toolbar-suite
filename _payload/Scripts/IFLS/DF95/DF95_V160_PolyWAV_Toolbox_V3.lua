...
-- @description DF95_V160 PolyWAV Toolbox V3 – ZoomF6 Mapping + SampleDB Bridge + Log
-- @author DF95
-- @version 1.0
-- @about
...
local function save_sampledb_multi_ucs(db, db_path)
  ...
  if not f then
    r.ShowMessageBox(
      ...
      "DF95 PolyWAV Toolbox V3 – SampleDB Bridge", 0)
    return false
  end
...
local function do_explode_multichannel(opt_enable_mapping, opt_enable_sampledb)
  ...
  r.Undo_EndBlock("DF95 PolyWAV Toolbox V3 – Explode + ZoomF6 + SampleDB", -1)
end
...
local function ensure_imgui()
  ...
  if not r.ImGui_CreateContext then
    r.ShowMessageBox(
      ...
      "DF95 PolyWAV Toolbox V3", 0)
    return nil
  end
  ctx = r.ImGui_CreateContext("DF95 PolyWAV Toolbox V3")
  return ctx
end
...
local function loop()
  ...
  local visible, open = r.ImGui_Begin(ctx, "DF95 PolyWAV Toolbox V3 – ZoomF6", true)
