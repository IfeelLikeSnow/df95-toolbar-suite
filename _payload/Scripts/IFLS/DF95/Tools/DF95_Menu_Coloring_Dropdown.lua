-- @description Coloring Selector (Dropdown)
-- @version 1.0
local r = reaper
...
local index = helper.readjson(res..sep.."Data"..sep.."DF95"..sep.."DF95_Coloring_Index.json")
local menu, entries, paths = helper.flat_menu_from_index(index)
r.gfx.init("DF95: Coloring Selector", 260, 38, 0, 400, 240)
local choice = r.gfx.showmenu(menu)
r.gfx.quit()
if choice>0 then
  local p = paths[choice]
  local fp = res..sep.."FXChains"..sep.."DF95"..sep.."Coloring"..sep..p.rel:gsub("/", sep)
  loader(fp, true)
end
